...
import { InterviewAgent } from '../agent/handler';
...
export const startInterview = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const studentToken = await generateParticipantToken(
      roomId, 
      studentId.toString(), 
      studentName,
...
export const downloadReportPDF = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.user) return res.status(401).json({ message: 'Unauthorized' });
    const { id } = req.params;
    let report = await Report.findOne({ interviewId: id });
