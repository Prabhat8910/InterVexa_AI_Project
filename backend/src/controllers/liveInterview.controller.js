...
import { LiveInterviewAgent } from '../agent/liveHandler.js';
import { generateLiveReport } from '../services/liveReport.service.js';
// Global registry to track active live agents so we can clean them up programmatically
export const activeLiveAgents = new Map();
export const createLiveRoom = async (req, res, next) => {
    try {
        ...
        const creatorToken = await generateParticipantToken(roomId, userId.toString(), userName, JSON.stringify({ role, userId: userId.toString() }));
        // 3. Generate token for the silent AI Agent
        const agentToken = await generateParticipantToken(roomId, 'ai-evaluation-agent', 'InterVexa AI Observer', JSON.stringify({ role: 'agent' }));
...
export const joinLiveRoom = async (req, res, next) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        const token = await generateParticipantToken(roomId, userId.toString(), userName, JSON.stringify({ role, userId: userId.toString() }));
