...
export const analyzeSpeechData = (
  transcription: string,
  audioDurationSec: number,
  volumeFluctuations: number[]
): VoiceMetrics => {
  ...
  let communication = 8;
  if (fillersCount > 2) communication -= 2;
  if (speedWordsPerMin > 160 || (speedWordsPerMin < 70 && speedWordsPerMin > 0)) communication -= 1;
