import { Room, RoomEvent, AudioSource, LocalAudioTrack, AudioStream, AudioFrame, TrackPublishOptions, TrackSource, TrackKind } from '@livekit/rtc-node';
...
import { checkSemanticSimilarity } from './similarity.js';
export const isSkipOrMeaninglessAnswer = (answer) => {
...
export class InterviewAgent {
    ...
    resumeContext = '';
    conversationHistory = [];
    maxQuestions = 6; // Greeting + 5 technical rounds
    ...
    volumeThreshold = 500; // RMS threshold for active voice to filter ambient noise
    silenceThresholdMs = 3000; // 3.0s of silence triggers STT
    sampleRate = 16000;
    numChannels = 1;
            ...
            for (const [_, participant] of this.room.remoteParticipants) {
                for (const [_, publication] of participant.trackPublications) {
                    if (publication.track && publication.track.kind === TrackKind.KIND_AUDIO && participant.identity !== this.room.localParticipant?.identity) {
                        ...
                        this.handleStudentAudio(publication.track);
                    }
                }
            }
    ...
    async runGreeting() {
        try {
            ...
            await InterviewSession.findOneAndUpdate({ roomId: this.roomName }, {
                status: 'active',
    ...
    async handleStudentAudio(track) {
        try {
            ...
            let pcmBufferChunks = [];
            let preRollBuffer = []; // Pre-roll buffer to prevent onset clipping (300ms history)
    ...
    async processTurn(pcmBuffer, volumeHistory) {
        try {
            ...
            const isGoodbyeMessage = lowerQuestion.includes('goodbye') ||
    ...
    async queryLLMEvaluation(userAnswer, isLastQuestion) {
        try {
    ...
    async speak(text) {
        try {
            ...
            while (offset < pcmData.length) {
                ...
                const cleanArrayBuffer = chunk.buffer.slice(chunk.byteOffset, chunk.byteOffset + chunk.byteLength);
    ...
    async generateReport(sessionId) {
      ...
      `;
            ...
            const expectedAnswersComparison = (parsedData.expectedAnswers || []).map((ea) => {
...
Detailed Review: ${ea.evaluationFeedback || 'N/A'}`;
            ...
            session.reportId = savedReport._id;
            await session.save();
