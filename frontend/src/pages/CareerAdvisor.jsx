...
import { Map, Sparkles, BookOpen, FolderGit2, Bookmark, AlertCircle } from 'lucide-react';
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1 rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur flex flex-col justify-between min-h-[300px]">
          <div className="space-y-4">
            ...
            <div>
              ...
              <select value={targetRole} onChange={(e) => setTargetRole(e.target.value)} className="w-full rounded-xl border border-white/10 bg-[#0b0f19] py-3 px-4 text-sm text-white outline-none transition focus:border-brandPrimary">
                {roles.map(r => (<option key={r} value={r}>{r}</option>))}
              </select>
            </div>

            {error && (<div className="flex items-start space-x-2 text-xs text-red-400 rounded-xl bg-red-500/10 border border-red-500/25 p-3">
                <AlertCircle className="h-4 w-4 shrink-0"/>
                <span>{error}</span>
              </div>)}
          </div>

          <button onClick={handleGenerate} disabled={isGenerating} className="mt-6 w-full rounded-xl bg-brandPrimary py-3 font-semibold text-white transition hover:bg-brandPrimary/80 disabled:opacity-50">
        ...
        <div className="lg:col-span-2 space-y-6">
          {roadmap ? (<div className="space-y-6">
              
              {/* Placement Strategy box */}
              {roadmap.placementPrepPlan && (<div className="rounded-2xl border border-white/10 bg-gradient-to-r from-brandPrimary/10 to-brandSecondary/5 p-6">
              ...
              <div className="space-y-6">
                {roadmap.roadmapSteps?.map((step, idx) => (<div key={idx} className="relative pl-8 border-l border-white/10">
                    {/* Circle marker */}
                    ...
                    <div className="rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur space-y-4">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {step.recommendedSkills?.map((skill) => (<span key={skill} className="rounded bg-white/5 border border-white/10 px-2.5 py-0.5 text-xs text-gray-300">
                              {skill}
                            </span>))}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            {step.courses?.map((course, cIdx) => (<div key={cIdx} className="bg-white/2 border border-white/5 rounded-xl p-3 text-xs">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            {step.suggestedProjects?.map((proj, pIdx) => (<div key={pIdx} className="bg-white/2 border border-white/5 rounded-xl p-3 text-xs">
                      ...
                      {/* Certifications */}
                      {step.certifications?.length > 0 && (<div className="border-t border-white/5 pt-3">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {step.certifications.map((cert) => (<span key={cert} className="rounded bg-brandSecondary/10 border border-brandSecondary/20 px-2 py-0.5 text-xs text-brandSecondary">
                                {cert}
                              </span>))}
                          </div>
                        </div>)}
            ...
            </div>) : (<div className="rounded-2xl border border-white/10 bg-cardBg p-12 text-center backdrop-blur text-textMuted text-sm">
