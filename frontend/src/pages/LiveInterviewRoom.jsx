...
import { Mic, MicOff, PhoneOff, Clock, Brain, Users, Copy, Check, Video, MessageSquare, Activity, AlertCircle } from 'lucide-react';
export const LiveInterviewRoom = () => {
    ...
    const [selectedRole, setSelectedRole] = useState((paramRole === 'candidate' || paramRole === 'interviewer') ? paramRole : 'interviewer');
    ...
    const handleConnect = async (token, roomName, seshId, role) => {
    ...
    const updateParticipants = (room) => {
        ...
        for (const [_, participant] of room.remoteParticipants) {
            ...
            try {
                ...
                if (participant.identity === 'ai-evaluation-agent')
                    role = 'agent';
            }
    ...
    const updateParticipantMuteState = (participant, isMuted) => {
        let role = 'candidate';
        try {
            const metadata = participant.metadata ? JSON.parse(participant.metadata) : {};
            role = metadata.role || 'candidate';
    ...
    const setupMicrophoneVisualizer = (track) => {
        try {
    ...
    const drawWave = () => {
        ...
        const renderFrame = () => {
            ...
            if (agentState === 'interviewer_speaking')
                waveColor = '#ec4899'; // Interviewer speaking pink
            else if (agentState === 'candidate_speaking')
                waveColor = '#6366f1'; // Candidate speaking indigo
            else if (agentState === 'interviewer_thinking' || agentState === 'candidate_thinking')
                waveColor = '#eab308'; // AI agent processing yellow
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur">
        ...
        {sessionStarted && roomId && (<div className="flex items-center space-x-2">
            <span className="text-xs font-mono bg-white/5 border border-white/10 px-3 py-1.5 rounded-xl text-gray-300">
              Room ID: {roomId}
            </span>
            <button onClick={copyRoomId} className="flex items-center justify-center h-9 w-9 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-gray-300 transition" title="Copy Room ID">
              {isCopied ? <Check className="h-4 w-4 text-brandSecondary"/> : <Copy className="h-4 w-4"/>}
            </button>
          </div>)}
      </div>

      {error && (<div className="flex items-center space-x-2 rounded-xl bg-red-500/10 border border-red-500/20 p-4 text-sm text-red-400">
          <AlertCircle className="h-5 w-5 flex-shrink-0"/>
          <span>{error}</span>
        </div>)}

      {/* 2. Setup Screen (when not connected) */}
      {!sessionStarted && !isEvaluating && (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => setSelectedRole('interviewer')} className={`flex flex-col items-center justify-center p-4 rounded-xl border transition ${selectedRole === 'interviewer'
                ...
                : 'border-white/5 bg-white/2 text-gray-400 hover:bg-white/5'}`}>
                  ...
                  <button onClick={() => setSelectedRole('candidate')} className={`flex flex-col items-center justify-center p-4 rounded-xl border transition ${selectedRole === 'candidate'
                ? 'border-brandPrimary bg-brandPrimary/10 text-white shadow-lg shadow-brandPrimary/5'
                : 'border-white/5 bg-white/2 text-gray-400 hover:bg-white/5'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-white/5">
                ...
                <div className="space-y-3">
                  ...
                  <button onClick={handleCreateRoom} disabled={isConnecting} className="w-full flex items-center justify-center space-x-2 rounded-xl bg-gradient-to-r from-brandPrimary to-brandSecondary px-4 py-3 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-50">
                ...
                <div className="space-y-3">
                  ...
                  <input type="text" value={roomIdInput} onChange={(e) => setRoomIdInput(e.target.value)} placeholder="live-interview-xxxx..." className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-brandPrimary focus:outline-none"/>
                  <button onClick={handleJoinRoom} disabled={isConnecting} className="w-full flex items-center justify-center space-x-2 rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-sm font-semibold text-white transition hover:bg-white/10 disabled:opacity-50">
      ...
      {isEvaluating && (<div className="rounded-2xl border border-white/10 bg-cardBg p-12 text-center backdrop-blur space-y-6 max-w-xl mx-auto">
      ...
      {sessionStarted && !isEvaluating && (<div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                  ...
                  <div className={`p-5 rounded-xl border transition ${agentState === 'interviewer_speaking'
                ? 'bg-pink-500/5 border-pink-500/30'
                : 'bg-white/2 border-white/5'}`}>
                    <div className="flex items-center space-x-3 mb-4">
                      <div className={`h-12 w-12 rounded-xl flex items-center justify-center text-white ${interviewerConnected ? 'bg-pink-600/30 border border-pink-500/40' : 'bg-white/5 border border-white/10'}`}>
                    ...
                    <div className="flex items-center justify-between text-xs mt-4 pt-3 border-t border-white/5">
                      ...
                      {interviewerMuted ? (<span className="text-red-400 font-semibold flex items-center space-x-1">
                          <MicOff className="h-3.5 w-3.5"/>
                          <span>Muted</span>
                        </span>) : (<span className="text-brandAccent font-semibold flex items-center space-x-1">
                  ...
                  <div className={`p-5 rounded-xl border transition ${agentState === 'candidate_speaking'
                ? 'bg-brandPrimary/5 border-brandPrimary/30'
                : 'bg-white/2 border-white/5'}`}>
                    <div className="flex items-center space-x-3 mb-4">
                      <div className={`h-12 w-12 rounded-xl flex items-center justify-center text-white ${candidateConnected ? 'bg-brandPrimary/30 border border-brandPrimary/40' : 'bg-white/5 border border-white/10'}`}>
                    ...
                    <div className="flex items-center justify-between text-xs mt-4 pt-3 border-t border-white/5">
                      <span className="text-textMuted">Microphone</span>
                      {candidateMuted ? (<span className="text-red-400 font-semibold flex items-center space-x-1">
                          <MicOff className="h-3.5 w-3.5"/>
                          <span>Muted</span>
                        </span>) : (<span className="text-brandAccent font-semibold flex items-center space-x-1">
                          <Mic className="h-3.5 w-3.5"/>
              ...
              <div className="rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button onClick={toggleMute} className={`flex h-11 w-11 items-center justify-center rounded-xl border transition ${isMuted
                ? 'bg-red-500/10 border-red-500/30 text-red-400 hover:bg-red-500/20'
                : 'bg-white/5 border-white/10 text-gray-300 hover:bg-white/10'}`} title={isMuted ? 'Unmute microphone' : 'Mute microphone'}>
                    {isMuted ? <MicOff className="h-5 w-5"/> : <Mic className="h-5 w-5"/>}
                  </button>

                  {/* End Interview Call Button */}
                  <button onClick={handleEndInterview} className="flex items-center justify-center space-x-2 rounded-xl bg-red-600 hover:bg-red-500 px-5 py-3 text-sm font-semibold text-white shadow shadow-red-600/20 transition">
            ...
            <div className="rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur flex flex-col h-[495px]">
              ...
              <div className="flex-1 overflow-y-auto space-y-4 pr-1 scrollbar-thin">
                {transcripts.length === 0 ? (<div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-3">
                  ...
                  </div>) : (transcripts.map((t, idx) => (<div key={idx} className={`p-3 rounded-xl border text-xs space-y-1.5 transition leading-relaxed ${t.sender === 'interviewer'
