...
import { Mic, MicOff, PhoneOff, Clock, Sparkles, MessageSquare, AlertCircle, Brain } from 'lucide-react';
export const InterviewRoom = () => {
    ...
    const [aiState, setAiState] = useState('greeting');
    ...
    const aiStateRef = useRef('greeting');
    // LiveKit refs
    ...
    const drawWave = () => {
        ...
        const renderFrame = () => {
            ...
            if (aiState === 'speaking')
                pulseColor = '#EC4899'; // Pink when AI speaks
            else if (aiState === 'thinking')
                pulseColor = '#EAB308'; // Yellow when thinking
    ...
    return (<div className="space-y-6">
      {isEvaluating ? (
        ...
        <div className="mx-auto max-w-2xl rounded-2xl border border-white/10 bg-cardBg p-8 text-center backdrop-blur shadow-2xl space-y-6">
          ...
          <div className="pt-4">
            <button onClick={() => navigate('/dashboard')} className="text-xs text-textMuted underline hover:text-white transition">
        ...
        <div className="mx-auto max-w-2xl rounded-2xl border border-white/10 bg-cardBg p-8 text-center backdrop-blur shadow-2xl">
          ...
          {error && (<div className="mt-6 flex items-start space-x-2 rounded-xl bg-red-500/10 border border-red-500/25 p-4 text-sm text-red-400 text-left max-w-sm mx-auto">
              <AlertCircle className="h-5 w-5 shrink-0"/>
              <span>{error}</span>
            </div>)}

          <button onClick={handleStartInterview} disabled={isConnecting} className="mt-8 rounded-xl bg-brandPrimary px-8 py-4 font-semibold text-white shadow-lg shadow-brandPrimary/30 transition hover:bg-brandPrimary/80 disabled:opacity-50 font-sans">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="relative rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur flex flex-col justify-between min-h-[400px]">
              ...
              <div className="flex-grow flex flex-col items-center justify-center my-6">
                <canvas ref={canvasRef} width={300} height={100} className="w-full max-w-sm rounded-lg"/>
              ...
              <div className="flex items-center justify-center space-x-4 border-t border-white/5 pt-4">
                <button onClick={toggleMute} className={`flex h-12 w-12 items-center justify-center rounded-xl transition ${isMuted
                ? 'bg-red-500 text-white shadow-lg shadow-red-500/20'
                : 'bg-white/5 border border-white/10 text-gray-300 hover:bg-white/10'}`}>
                  {isMuted ? <MicOff className="h-5 w-5"/> : <Mic className="h-5 w-5"/>}
                </button>
                
                <button onClick={handleDisconnect} className="flex h-12 px-6 items-center justify-center space-x-2 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 font-semibold transition hover:bg-red-500/20">
            ...
            {speechMetrics && (<div className="rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur">
          ...
          <div className="rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur flex flex-col justify-between h-[540px]">
            ...
            <div className="flex-grow overflow-y-auto space-y-4 pr-2">
              {transcripts.map((t, idx) => (<div key={idx} className={`flex flex-col max-w-[85%] rounded-2xl p-4 text-sm ${t.sender === 'ai'
