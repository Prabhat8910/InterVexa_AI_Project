...
export const AdminDashboard: React.FC = () => {
  ...
  const handleDeleteUser = async (id: string) => {
    if (!window.confirm('Are you sure you want to delete this user? All profile, transcript, and report logs will be permanently deleted.')) return;
    try {
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur">
          ...
          <div className="space-y-4 overflow-y-auto max-h-[360px] pr-2">
            {logs.map((log, idx) => (
              <div key={idx} className="rounded-xl bg-white/2 border border-white/5 p-3 text-xs space-y-1">
