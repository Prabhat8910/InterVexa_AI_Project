...
export const RecruiterDashboard = () => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="relative w-full max-w-md">
        ...
        <input type="text" placeholder="Search by name, role, or skills..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full rounded-xl border border-white/10 bg-white/5 py-3 pl-11 pr-4 text-sm text-white placeholder-gray-500 outline-none transition focus:border-brandPrimary focus:bg-white/10"/>
      </div>

      {/* Candidates list table */}
      <div className="rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur">
        {filteredCandidates.length > 0 ? (<div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-sm divide-y divide-white/5">
                {filteredCandidates.map((candidate) => (<tr key={candidate.id} className="hover:bg-white/1">
                    <td className="py-4">
                    ...
                    <td className="py-4">
                      <span className={`text-xs font-semibold rounded-full px-2.5 py-0.5 border ${candidate.aiRecommendation === 'Highly Recommended'
                ...
                }} disabled={!candidate.latestReportId} className={`flex items-center space-x-1 text-xs font-semibold ${candidate.latestReportId
