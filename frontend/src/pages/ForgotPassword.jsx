...
export const ForgotPassword = () => {
    ...
    return (<div className="flex min-h-[80vh] items-center justify-center px-4 py-16 bg-darkBg text-white">
      <div className="w-full max-w-md rounded-2xl glass-panel p-8 shadow-2xl pulse-glow">
        ...
        {successMessage && (<div className="mb-6 flex items-start space-x-2 rounded-xl bg-emerald-500/10 border border-emerald-500/25 p-4 text-sm text-emerald-400">
            <CheckCircle2 className="h-5 w-5 shrink-0"/>
            <span>{successMessage}</span>
          </div>)}

        {/* Dev Mode Link Simulator Helper */}
        {devResetLink && (<div className="mb-6 rounded-xl bg-amber-500/10 border border-amber-500/25 p-4 text-sm text-amber-300">
            ...
            <a href={devResetLink.replace('http://localhost:5173', window.location.origin)} className="inline-flex w-full items-center justify-center rounded-lg bg-amber-500/20 hover:bg-amber-500/35 border border-amber-500/40 py-2 px-3 text-xs font-semibold text-white transition tracking-wide text-center">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <div>
            ...
            <div className="relative">
              ...
              <input type="email" placeholder="you@example.com" {...register('email')} disabled={!!successMessage} className="w-full rounded-xl border border-white/10 bg-white/5 py-3 pl-11 pr-4 text-sm text-white placeholder-gray-500 outline-none transition focus:border-brandPrimary focus:bg-white/10 disabled:opacity-50"/>
            </div>
            {errors.email && (<p className="mt-1 text-xs text-red-400">{errors.email.message}</p>)}
          </div>

          <button type="submit" disabled={isSubmitting || !!successMessage} className="flex w-full items-center justify-center space-x-2 rounded-xl bg-gradient-to-r from-brandPrimary to-brandSecondary py-3 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-50">
            {isSubmitting ? (<span>Sending request...</span>) : (<span>Send Reset Link</span>)}
          </button>
        </form>
