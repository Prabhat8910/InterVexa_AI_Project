      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-1 rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur flex flex-col justify-between min-h-[300px]">
          <div>
            ...
            <div 
              ...
              onDrop={handleDrop}
              className="border-2 border-dashed border-white/10 rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer transition hover:border-brandPrimary/40 hover:bg-white/1"
            >
              ...
              <label 
                ...
                className="mt-4 rounded-xl bg-white/5 border border-white/10 px-4 py-2 text-xs font-semibold text-white transition hover:bg-white/10 cursor-pointer"
              >
                Select File
              </label>
            </div>
            {file && (
              <div className="mt-4 flex items-center space-x-2 text-sm text-brandAccent">
                <CheckCircle className="h-4 w-4 shrink-0" />
                <span className="truncate max-w-[200px]">{file.name}</span>
              </div>
            )}
            {error && (
              <div className="mt-4 flex items-start space-x-2 text-xs text-red-400">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-cardBg p-6 backdrop-blur min-h-[300px]">
          {analysis ? (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div className="rounded-xl bg-white/2 border border-white/5 p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {analysis.skillsExtract?.identified?.map((s: string) => (
                      <span key={s} className="rounded bg-brandPrimary/10 border border-brandPrimary/20 px-2 py-1 text-xs font-semibold text-brandPrimary">
                        {s}
                      </span>
                    )) || <span className="text-textMuted text-xs">None identified</span>}
                  </div>
                </div>
                ...
                <div className="rounded-xl bg-white/2 border border-white/5 p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {analysis.skillsExtract?.missing?.map((s: string) => (
                      <span key={s} className="rounded bg-red-500/10 border border-red-500/20 px-2 py-1 text-xs font-semibold text-red-400">
                        {s}
                      </span>
                    )) || <span className="text-textMuted text-xs">None missing</span>}
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {analysis.roleMatches?.map((match: any) => (
                    <div key={match.roleName} className="rounded-xl bg-white/2 border border-white/5 p-4">
              ...
              {analysis.grammarSuggestions?.length > 0 && (
                <div>
                  ...
                  <div className="space-y-3">
                    {analysis.grammarSuggestions.map((g: any, idx: number) => (
                      <div key={idx} className="rounded-xl bg-white/2 border border-white/5 p-4 text-xs space-y-1">
            ...
            <div className="flex h-full flex-col items-center justify-center text-textMuted text-sm py-12">
