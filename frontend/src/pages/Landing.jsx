...
export const Landing = () => {
    ...
    return (<div className="relative min-h-screen bg-darkBg text-white overflow-hidden">
      {/* Background Orbs */}
      ...
      <section className="relative mx-auto max-w-7xl px-6 pt-24 pb-20 text-center md:pt-32">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="inline-flex items-center space-x-2 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-gray-300 mb-6">
          <Zap className="h-3 w-3 text-brandSecondary animate-pulse"/>
          <span>Driven by LiveKit & Groq API</span>
        </motion.div>

        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="text-4xl font-extrabold tracking-tight text-white sm:text-6xl md:text-7xl font-sans">
          Accelerate Your Placement <br />
          <span className="gradient-text">Preparation with Voice AI</span>
        </motion.h1>

        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4, duration: 0.8 }} className="mx-auto mt-6 max-w-2xl text-lg text-textMuted md:text-xl">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.6 }} className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <Link to="/register" className="flex items-center space-x-2 rounded-xl bg-brandPrimary px-8 py-4 font-semibold text-white shadow-lg shadow-brandPrimary/30 transition hover:bg-brandPrimary/80">
            <span>Start Mock Interview</span>
            <ArrowRight className="h-5 w-5"/>
          </Link>
          <a href="#ecosystem" className="rounded-xl border border-white/10 bg-white/5 px-8 py-4 font-semibold text-white transition hover:bg-white/10">
            Explore Ecosystem
          </a>
        </motion.div>
      </section>
      ...
      <section id="ecosystem" className="mx-auto max-w-7xl px-6 py-20">
        ...
        <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-100px' }} className="grid grid-cols-1 gap-8 md:grid-cols-3">
          {ecosystems.map((eco) => (<motion.div key={eco.title} variants={itemVariants} className="rounded-2xl glass-panel glass-panel-hover p-8 relative flex flex-col justify-between">
              <div>
                ...
                <ul className="space-y-3.5 mb-8">
                  {eco.features.map((feat) => (<li key={feat} className="flex items-start space-x-3 text-sm text-gray-300">
              ...
              <Link to="/register" className="flex items-center space-x-1.5 text-sm font-semibold text-brandPrimary hover:text-white transition">
