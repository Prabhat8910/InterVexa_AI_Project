...
export const Sidebar = () => {
    ...
    if (user.role === 'admin')
        menuItems = adminMenu;
    return (<aside className="fixed bottom-0 top-0 left-0 z-30 hidden w-64 border-r border-white/10 bg-darkBg/60 backdrop-blur-md px-6 py-8 md:flex md:flex-col justify-between">
      <div>
        <div className="flex items-center space-x-2 px-2 pb-8 cursor-pointer" onClick={() => navigate('/')}>
          ...
          <span className="text-lg font-bold text-white tracking-tight">InterVexa</span>
        </div>

        <nav className="space-y-1">
          {menuItems.map((item) => (<NavLink key={item.name} to={item.path} end={item.path === '/dashboard' || item.path === '/recruiter' || item.path === '/university' || item.path === '/admin'} className={({ isActive }) => `flex items-center justify-between rounded-xl px-4 py-3 text-sm font-medium transition ${isActive
                ? 'bg-gradient-to-r from-brandPrimary/20 to-brandSecondary/10 text-white border border-brandPrimary/30 shadow-lg shadow-brandPrimary/10'
                : 'text-gray-400 border border-transparent hover:text-white hover:bg-white/5'}`}>
              <div className="flex items-center space-x-3">
                {item.icon}
                <span>{item.name}</span>
              </div>
              {('badge' in item) && item.badge && (<span className="rounded bg-brandSecondary/25 border border-brandSecondary/40 px-1.5 py-0.5 text-[9px] font-bold text-brandSecondary uppercase tracking-wider animate-pulse">
                  {item.badge}
                </span>)}
            </NavLink>))}
        </nav>
      ...
      <div className="border-t border-white/10 pt-6">
        <div className="flex items-center space-x-3 px-2 mb-6">
          ...
          <div>
            <h4 className="text-sm font-medium text-white truncate max-w-[140px]">{user.name}</h4>
            <span className="text-xs text-textMuted capitalize">{user.role}</span>
          </div>
        </div>

        <button onClick={logout} className="flex w-full items-center space-x-3 rounded-xl border border-transparent px-4 py-3 text-sm font-medium text-red-400 transition hover:bg-red-500/10 hover:border-red-500/20">
