...
export const Navbar = () => {
    ...
    return (<nav className="sticky top-0 z-50 w-full border-b border-white/10 bg-darkBg/60 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <div className="flex items-center space-x-4">
          {user ? (<>
              <button onClick={handleDashboardRedirect} className="flex items-center space-x-2 rounded-xl bg-white/5 border border-white/10 px-4 py-2 text-sm font-medium text-white transition hover:bg-white/10">
                <LayoutDashboard className="h-4 w-4"/>
                <span>Dashboard</span>
              </button>
              <button onClick={logout} className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-gray-300 transition hover:bg-red-500/20 hover:text-red-400 hover:border-red-500/30">
              ...
              <Link to="/register" className="rounded-xl bg-brandPrimary px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-brandPrimary/20 transition hover:bg-brandPrimary/80">
                Get Started
              </Link>
            </>)}
        </div>
